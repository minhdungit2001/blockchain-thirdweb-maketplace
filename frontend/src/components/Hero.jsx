...
export default function Hero() {
  ...
  const { setGlobalState, isMyNfts } = useStateContext();
    ...
    >
      <div className="md:w-3/6 w-full">
        ...
        <div className="flex flex-row mt-5">
          <button
            className="shadow-xl shadow-black text-white bg-[#e32970] hover:bg-[#bd255f] rounded-full cursor-pointer p-2 mr-4"
          ...
          <button
            className="shadow-xl shadow-black text-white bg-[#e32970] hover:bg-[#bd255f] rounded-full cursor-pointer p-2 pl-10 pr-10"
            onClick={() => {
              setGlobalState("isMyNfts", !isMyNfts);
            }}
          >
            {!isMyNfts ? "My Nfts" : "Marketplace"}
          </button>
        </div>
      ...
      <div className="shadow-xl shadow-black md:w-2/5 w-full mt-10 md:mt-0 rounded-md overflow-hidden bg-gray-800">
        ...
        <div className="flex justify-start items-center p-3">
          <Identicon
            string={address ? address : "Connect Your Wallet"}
            size={50}
            className="h-10 w-10 object-contain rounded-full mr-3"
          />
          <div>
            <p className="text-white font-semibold">
              {address ? truncate(address, 4, 4, 11) : "Connect Your Wallet"}
            </p>
