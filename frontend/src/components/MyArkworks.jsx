...
export default function Artworks() {
  const { setGlobalState, myNfts } = useStateContext();
  const count = 4;
  ...
  useEffect(() => {
    setCollection(() => myNfts.slice(0, end));
  }, [myNfts, end]);
    ...
    <div className="bg-[#151c25] gradient-bg-artworks">
      <div className="w-4/5 py-10 mx-auto">
        <h4 className="text-white text-3xl font-bold uppercase text-gradient">
          {collection.length > 0 ? "My Artworks" : "No Artworks"}
        </h4>
        ...
        {collection.length > 0 && myNfts.length > collection.length ? (
